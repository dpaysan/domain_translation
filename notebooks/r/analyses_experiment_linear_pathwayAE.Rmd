...
```{r}
pathway.activities <- read.csv("~/PycharmProjects/domain_translation/data/cd4/experiments/analyses_of_current_best/pathway_activities.csv", header = T, row.names = 1)
# The cell label was used to guide the learning of the latent space.
cell.labels <- pathway.activities$label
pathway.activities <- pathway.activities[,1:(NCOL(pathway.activities)-1)]
...
pheatmap(log(t(as.matrix(pathway.activities[,rownames(dpa.results[order(dpa.results[,"fdr"]),])[1:10]][order(-cell.labels),]))+1e-20), show_rownames = T, show_colnames = F, scale = "row", cluster_rows = T, cluster_cols = F, gaps_col = 667, fontsize_row = 12)
```
...
get_silent_active_cells_for_pathway <- function(pathway, pathway.activities){
  silent.cells.ids <- pathway.activities[,pathway] == 0
...
get_dge_genes_for_pathway_active_vs_silent <- function(pathway, pathway.activities, seq.data, fdr = 0.05){
  ...
  if(min(length(silent.active.cells$silent.cells), length(silent.active.cells$active.cells)) > 5){
     dge.pathway <- FindMarkers(object = seq.data, ident.1 = silent.active.cells$silent.cells, ident.2 = silent.active.cells$active.cells, logfc.threshold = 0.05)
...
```{r}
pathway.geneset.ora.results.active.silent <- get_dge_genes_for_all_pathways_active_vs_silent(pathway.activities = pathway.activities, pathway.genes.list = pathway.genes.list, seq.data = seq.data, fdr = 0.05)
```
...
```{r}
plot(1:20, top_n_accs, xlab = "n", ylim=c(0.8,1), type = 'b', ylab="Top-n accuracy", main="Top-n accuracy of the pathway activity score specificity")
```
