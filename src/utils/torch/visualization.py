...
def visualize_geneset_perturbation_in_image(
    data_dict: dict, output_dir: str, silencing_node: int
):
    image_dir = os.path.join(
        output_dir, "perturbation/silenced_set_{}".format(silencing_node)
    )
    ...
    for i in range(len(trans_images)):
        imageio.imwrite(
            os.path.join(image_dir, "trans_image_%s.jpg" % i),
            np.uint8(trans_images[i].squeeze() * 255),
        )
        imageio.imwrite(
            os.path.join(image_dir, "perturbed_trans_image_%s.jpg" % i),
            np.uint8(perturbed_trans_images[i].squeeze() * 255),
        )
        plt.figure()
        plt.imshow(trans_images[i].squeeze() * 255, cmap="gray")
        plt.imshow(
            (perturbed_trans_images[i].squeeze() - trans_images[i].squeeze()) * 255,
            cmap="seismic",
            alpha=0.3,
        )
        plt.axis("off")
        plt.savefig(
            os.path.join(image_dir, "perturbed_diff_image_%s.jpg" % i),
            bbox_inches="tight",
        )
        plt.close()


def visualize_geneset_guided_grad_cams(
    data_dict: dict, output_dir: str, query_node: int
):
    grad_cams = data_dict["grad_cams"]
    gb_maps = data_dict["gb_maps"]
    images = data_dict["images"]
    image_dir = os.path.join(
        output_dir, "guided_gradcam/queried_set_{}".format(query_node)
    )
    os.makedirs(image_dir)
